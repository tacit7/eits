...
## 9. Implementation Notes (Task 10 final integration pass)

- **Regression found and fixed**: `test/eye_in_the_sky_web/live/project_live/sessions_test.exs` (New Session drawer, actually backed by `NewSessionModal` per Task 9) hardcoded `"model" => "claude-sonnet-4-6"` as a submittable form override and asserted on a native `select[name='model']` element. Task 1's catalog refresh changed `normalize_model_alias("sonnet")` from `"claude-sonnet-4-6"` to `"claude-sonnet-5"`, and Task 9 replaced the native select with the shared component's read-only hidden-input mirror — so `Phoenix.LiveViewTest`'s hidden-field value constraint correctly started rejecting the stale override (`"value for hidden \"model\" must be one of [\"claude-sonnet-5\"]"`). Fixed by dropping the model override where the test didn't care which model was selected, and driving the real `model_and_provider_selected` event (as the JS hook would) for the two tests that specifically needed a non-default model. 7 tests, 31/31 passing after the fix.
- **Confirmed unrelated, not fixed** (out of scope for this plan): `ProjectLive.PromptShowTest` (pre-existing `Ecto.ChangeError` — `Prompt.updated_at` schema type mismatch, unrelated to models), `EyeInTheSky.Terminal.PtyServerTest` (pre-existing `KeyError` on `:session_key`), `EyeInTheSky.Claude.SDKTest` (async message-timeout, appears flaky/pre-existing), `ProjectLive.AgentsTest` (a `top_bar_cta` title-attribute drift on a self-contained "Create New Agent" modal that has no model field at all and was never touched by any of Tasks 1–9). None of these intersect the file set this plan modified.
- **Full suite**: 4475+/4532 passing pre-fix, improved by the sessions_test.exs fix above; remaining failures are the four unrelated categories above, present regardless of this branch.
