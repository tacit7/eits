defmodule EyeInTheSky.Claude.SDK do
  @moduledoc """
  ...
  alias EyeInTheSky.Claude.{CLI, Message, Parser, Utils}
  ...
  defp run_session(mode, prompt, session_id, opts) do
    ...
    case spawn_handler_process(sdk_ref, to, task_supervisor) do
      ...
      {:error, reason} ->
        Logger.error(
          "[telemetry] sdk.error session_id=#{meta_session_id} reason=handler_start_failed #{inspect(reason)}"
        )
