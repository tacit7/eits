defmodule EyeInTheSky.IAM.Builtin.SanitizePromptApiKeys do
  @moduledoc """
  Detects and redacts API keys, access tokens, and credentials in UserPromptSubmit
  prompt input.
  ...
  @impl true
  def matches?(%Policy{}, %Context{prompt: prompt}) when is_binary(prompt) do
    has_secret?(prompt)
  end
  ...
  @impl true
  def instruction_message(%Policy{}, %Context{prompt: prompt})
      when is_binary(prompt) do
    {sanitized, _count, summary} = redact(prompt)
    "#{summary}\n\n#{sanitized}"
  end
