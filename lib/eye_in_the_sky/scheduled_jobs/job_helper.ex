defmodule EyeInTheSky.ScheduledJobs.JobHelper do
  ...
  def prompt(description, opts) do
    ...
    curl -s http://localhost:5001/api/v1/projects 2>/dev/null | jq '.projects[] | {id, name, path}' 2>/dev/null \
      || psql -d eits_dev -tAq -c "SELECT id, name FROM projects ORDER BY name;" 2>/dev/null
    curl -s http://localhost:5001/api/v1/jobs 2>/dev/null | jq '.jobs[] | {id, name, job_type, schedule_value}' 2>/dev/null \
    ...
    1. Create the Oban worker module at `lib/eye_in_the_sky/workers/<type>_worker.ex`:
    ```elixir
    defmodule EyeInTheSky.Workers.MyTypeWorker do
    ...
    2. Add the case to `enqueue_job/1` in `lib/eye_in_the_sky/scheduled_jobs.ex`:
    ```elixir
    "my_type" -> EyeInTheSky.Workers.MyTypeWorker
    ```
