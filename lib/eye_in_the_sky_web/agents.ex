defmodule EyeInTheSkyWeb.Agents do
  @moduledoc """
  The Agents context for managing autonomous execution units.

  An Agent represents an autonomous Claude process doing work (execution context),
  ...
  Pass `include_archived: true` to include archived execution agents.
  """
  def list_agents_for_chat_agent(chat_agent_id, opts \\ []) do
    Agent
  ...
  def create_execution_agent_with_model(attrs \\ %{}) do
    %Agent{}
    |> Agent.creation_changeset(attrs)
    |> Repo.insert()
  end
  ...
  """
  def update_execution_agent(%Agent{} = agent, attrs) do
  ...
  """
  def end_execution_agent(%Agent{} = agent) do
  ...
  """
  def archive_execution_agent(%Agent{} = agent) do
  ...
  """
  def delete_execution_agent(%Agent{} = agent) do
    Repo.delete(agent)
  end
  ...
  """
  def change_execution_agent(%Agent{} = agent, attrs \\ %{}) do
  ...
  Pass `include_archived: true` to include archived execution agents.
  """
  def list_active_agents(opts \\ []) do
    Agent
  ...
  Pass `include_archived: true` to include archived execution agents.
  """
  def list_agents_with_chat_agent(opts \\ []) do
    Agent
    |> preload(:chat_agent)
    |> order_by([ea], desc: ea.started_at)
    |> Archivable.include_archived(opts)
  ...
  """
  def list_agents_filtered(opts \\ []) do
  ...
  """
  def format_model_info(%Agent{} = agent) do
    raw =
