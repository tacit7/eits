defmodule EyeInTheSkyWeb.TopBar.Generic do
  ...
  def default_toolbar(assigns) do
    ...
    <button
      ...
      class="flex items-center gap-1.5 h-7 px-2.5 rounded-box text-mini font-medium text-base-content/45 hover:text-base-content/70 hover:bg-base-content/6 transition-colors"
      title="Search"
      aria-label="Search"
    >
      <.icon name="hero-magnifying-glass" class="size-3.5" /> Search
      <kbd class="ml-0.5 inline-flex items-center px-1 py-0.5 rounded-box text-nano bg-base-content/8 text-base-content/30 border border-base-content/10 font-sans leading-none">
        ⌘K
      </kbd>
    </button>
    """
