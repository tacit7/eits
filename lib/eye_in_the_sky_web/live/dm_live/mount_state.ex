defmodule EyeInTheSkyWeb.DmLive.MountState do
  ...
  defp assign_ui_flags(socket, session) do
    ...
    |> assign(:session_init_data, (session.settings || %{})["cli_init"])
  ...
  def assign_new_session_defaults(socket) do
    ...
    |> assign_new_session_ui_flags(effective)
    |> assign_stream_defaults()
    |> assign_task_defaults()
    |> assign_pty_defaults()
    |> assign_upload_config()
  ...
  defp assign_new_session_ui_flags(socket, effective) do
    socket
    |> assign(:hide_mobile_header, true)
    |> assign(:active_tab, "messages")
    |> assign(:session_ref, nil)
  ...
  defp assign_pty_defaults(socket) do
    socket
    |> assign(:pty_pid, nil)
