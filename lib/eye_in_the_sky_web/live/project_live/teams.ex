    ...
    >
      <div class="max-w-3xl mx-auto w-full">
        <div class="mb-3 flex items-center justify-between">
          <span class="text-mini font-mono tabular-nums text-base-content/45 uppercase">
          ...
          <%= if @project do %>
            <.link
              ...
              class={[
                "text-mini px-2 py-1 rounded-box transition-colors",
                if(@show_all,
        ...
        <% else %>
          <%= if MapSet.size(@selected_ids) > 0 do %>
            <% all_selected =
              MapSet.size(@selected_ids) == length(@all_teams) && length(@all_teams) > 0 %>
              ...
              >
                ...
                >
                  <.link
                    ...
                    class="flex-1 py-2.5 px-3 flex items-center gap-3 rounded-box hover:bg-base-200/40 transition-colors min-w-0 [&.vim-nav-focused]:ring-2 [&.vim-nav-focused]:ring-primary/50"
                    data-vim-list-item
                  >
                    <.status_dot status={team_status_atom(team.members)} size="sm" />
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <span class="text-message font-medium text-base-content/85 truncate">
                          {team.name}
                        </span>
                        <%= if team.status == "archived" do %>
                          <span class="text-micro text-base-content/35 font-medium">archived</span>
                        <% end %>
                      </div>
                      <div class="flex items-center gap-1.5 mt-0.5 text-micro text-base-content/40 font-mono">
                  ...
                  <button
                    ...
                    phx-value-id={team.id}
                    class="shrink-0 p-1.5 rounded-box text-base-content/20 hover:text-error/60 hover:bg-base-200 transition-colors opacity-0 group-hover:opacity-100 min-h-[36px] min-w-[36px] flex items-center justify-center"
                    title="Delete team"
                  >
