defmodule EyeInTheSkyWebWeb.OverviewLive.Config do
  ...
  def handle_params(params, _uri, socket) do
    ...
    case mode do
      ...
      :tree ->
        entries = scan_directory(@claude_dir, @claude_dir, 0)
  ...
  def handle_event("toggle_view_mode", %{"mode" => mode}, socket) do
    case mode do
      "list" -> {:noreply, push_patch(socket, to: ~p"/config?mode=list")}
      "tree" -> {:noreply, push_patch(socket, to: ~p"/config?mode=tree")}
      _ -> {:noreply, socket}
    end
  end
  ...
  def render(assigns) do
                  ...
                  <.link
                    patch={
                      if @current_path && Path.dirname(@current_path) != ".",
                        do: ~p"/config?mode=list&path=#{Path.dirname(@current_path)}",
                        else: ~p"/config?mode=list"
                    }
    ...
    <!-- Mobile list -->
                <div class="md:hidden space-y-2">
                  <%= for file <- @files do %>
                    <.link
                      patch={~p"/config?mode=list&path=#{file.path}"}
