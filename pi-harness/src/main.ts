...
async function startSession(message: RequestMessage): Promise<void> {
  ...
  const piIdentity = [
    "You are an AI coding agent running inside EITS, dispatched through the Pi SDK harness.",
    model?.id
    ...
    "Use the available tools normally; EITS will ask the user for approval before mutating commands or file changes.",
  ].join(" ");
...
async function handle(message: RequestMessage): Promise<void> {
  switch (message.type) {
    case "initialize": {
      ...
      const v = (message as any).protocolVersion;
      if (typeof v === "number" && v !== 1) {
        throw new Error(`unsupported protocolVersion ${v}; expected 1 — rebuild the harness: scripts/build-pi-harness.sh`);
      }
      respond(message.id, message.type, true, { version: PI_VERSION, protocolVersion: 1 });
      break;
    }
    case "start_session":
