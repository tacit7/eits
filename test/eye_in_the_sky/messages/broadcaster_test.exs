defmodule EyeInTheSky.Messages.BroadcasterTest do
  ...
  test "init tracks current max message id" do
    ...
    Application.put_env(:eye_in_the_sky, Broadcaster, enabled: true)
    {:ok, state} = Broadcaster.init([])
    Application.put_env(:eye_in_the_sky, Broadcaster, enabled: false)
  ...
  test "poll broadcasts to multiple sessions" do
    ...
    Phoenix.PubSub.subscribe(EyeInTheSky.PubSub, "session:#{s1.id}")
    Phoenix.PubSub.subscribe(EyeInTheSky.PubSub, "session:#{s2.id}")
