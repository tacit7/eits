defmodule EyeInTheSkyWeb.Components.DmPageTest do
  ...
  describe "mobile keyboard layout" do
    ...
    @messages_tab_source Path.join([
                           @project_root,
                           "lib/eye_in_the_sky_web/components/dm_page/messages_tab.ex"
                         ])
    ...
    test "attach label has mobile-adequate touch target" do
      ...
      # Attach button (label wrapping file input) should be w-10+ h-10+ on mobile, sm:w-8 sm:h-8 on desktop
      assert source =~ ~r/w-1[01] h-1[01] sm:w-8 sm:h-8/,
             "attach label must have adequate mobile touch target with sm:w-8 sm:h-8 on desktop"
    end
  ...
  describe "NewTaskDrawer placement" do
    @project_root Path.expand("../../..", __DIR__)
    @dm_page_source Path.join([@project_root, "lib/eye_in_the_sky_web/components/dm_page.ex"])
    @dm_live_source Path.join([@project_root, "lib/eye_in_the_sky_web/live/dm_live.ex"])
    @new_task_drawer_source Path.join([
                              @project_root,
                              "lib/eye_in_the_sky_web/components/new_task_drawer.ex"
                            ])
