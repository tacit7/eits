defmodule EyeInTheSkyWeb.OverviewLive.PromptsTest do
  ...
  describe "handle_params/3" do
    ...
    test "converts string id to integer for lookup" do
      ...
      socket = socket_with(%{prompts: [p1, p2]})
  ...
  describe "handle_event/3 - sort_prompts" do
    test "updates sort_by to name_asc" do
      socket = socket_with(%{sort_by: "recent"})
    ...
    test "updates sort_by to name_desc" do
      ...
      {:noreply, result} = PromptsLive.handle_event("sort_prompts", %{"by" => "name_desc"}, socket)
  ...
  describe "handle_event/3 - filter_scope" do
    test "updates scope_filter to global" do
      ...
      {:noreply, result} = PromptsLive.handle_event("filter_scope", %{"scope" => "global"}, socket)
    ...
    test "updates scope_filter to project" do
      ...
      {:noreply, result} = PromptsLive.handle_event("filter_scope", %{"scope" => "project"}, socket)
  ...
  describe "handle_event/3 - select_prompt" do
    test "selects a prompt" do
      p = prompt(id: 1, slug: "test")
      socket = socket_with(%{prompts: [p], selected_prompt: nil})
    ...
    test "deselects prompt when clicking the same id again" do
      p = prompt(id: 1, slug: "test")
      socket = socket_with(%{prompts: [p], selected_prompt: p})
    ...
    test "switches selected_prompt to a different prompt" do
      p1 = prompt(id: 1, slug: "test-1")
      p2 = prompt(id: 2, slug: "test-2")
      socket = socket_with(%{prompts: [p1, p2], selected_prompt: p1})
    ...
    test "resets detail_tab to preview when selecting a new prompt" do
      p = prompt(id: 1, slug: "test")
      socket = socket_with(%{prompts: [p], selected_prompt: nil, detail_tab: :raw})
    ...
    test "handles string id parameter" do
      p = prompt(id: 42, slug: "test")
      socket = socket_with(%{prompts: [p], selected_prompt: nil})
  ...
  describe "handle_event/3 - set_notify_on_stop" do
    test "forwards to NotificationHelpers.set_notify_on_stop without crashing" do
      ...
      result = PromptsLive.handle_event("set_notify_on_stop", %{}, socket)
