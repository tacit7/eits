defmodule EyeInTheSkyWeb.ProjectLive.KanbanTest do
  ...
  describe "new task drawer" do
    test "opens drawer on New Task click", %{conn: conn} do
      ...
      view
      |> element("button.ml-auto[phx-click='toggle_new_task_drawer']", "New Task")
      |> render_click()
  ...
  describe "PubSub" do
    test "ignores :tasks_changed broadcast for a different project", %{conn: conn} do
      ...
      Phoenix.PubSub.broadcast(EyeInTheSky.PubSub, "tasks:#{other_project.id}", :tasks_changed)
    ...
    test "reloads tasks on :tasks_changed for correct project", %{conn: conn} do
      ...
      Phoenix.PubSub.broadcast(EyeInTheSky.PubSub, "tasks:#{project.id}", :tasks_changed)
