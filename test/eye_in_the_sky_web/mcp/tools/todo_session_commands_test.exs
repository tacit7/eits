defmodule EyeInTheSkyWeb.MCP.Tools.TodoSessionCommandsTest do
  ...
  test "remove-session: unlinks a session from a task" do
    ...
    r = Todo.execute(%{command: "remove-session", task_id: tid(task), session_id: session.uuid}, @frame) |> json_result()
  ...
  test "remove-session: succeeds silently for unknown task (0 rows deleted)" do
    session = new_session()
    r = Todo.execute(%{command: "remove-session", task_id: "999999", session_id: session.uuid}, @frame) |> json_result()
  ...
  test "remove-session: error for unknown session" do
    task = make_task()
    r = Todo.execute(%{command: "remove-session", task_id: tid(task), session_id: "ghost-uuid"}, @frame) |> json_result()
  ...
  test "add-session-to-tasks: idempotent — linking twice doesn't error" do
    ...
    Todo.execute(%{command: "add-session-to-tasks", session_id: session.uuid, task_ids: [tid(task)]}, @frame)
    r = Todo.execute(%{command: "add-session-to-tasks", session_id: session.uuid, task_ids: [tid(task)]}, @frame) |> json_result()
  ...
  test "add-session-to-tasks: error for unknown session" do
    task = make_task()
    r = Todo.execute(%{command: "add-session-to-tasks", session_id: "ghost-uuid", task_ids: [tid(task)]}, @frame) |> json_result()
    assert r.success == false
  end

  test "add-session-to-tasks: empty task_ids list returns error" do
    session = new_session()
    r = Todo.execute(%{command: "add-session-to-tasks", session_id: session.uuid, task_ids: []}, @frame) |> json_result()
    assert r.success == false
  ...
  test "reindex: returns unsupported error" do
    ...
    assert r.success == false
    assert String.contains?(r.message, "unsupported") or String.contains?(r.message, "not supported")
  end
